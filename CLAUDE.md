...
Tested: 85 backend tests (incl. `test_clients.py`, `test_tamper.py`, also green on PostgreSQL via `TEST_DATABASE_URL`); live tamper scenarios (modified cache, cloned identity, stolen token, edited state/code); clients/fleet/security pages in headless Chromium; the previous version's DB upgraded on SQLite and Postgres; live agents (geofence switching, offline/reconnect, emergency applied on reconnect, impressions, WebM playback from MinIO); full Docker stack with the sim profile;
