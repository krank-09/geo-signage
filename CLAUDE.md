...
- **Multi-client isolation** (`scope.py`, `api/clients.py`, `services/clients.py`): every resource has `client_id`. `Scope` is resolved per request: client users are pinned to their own client (`X-Client-Id` ignored); platform users (`User.client_id` NULL) may send `X-Client-Id`, otherwise they see all clients, or the only client if just one exists.
  Reads use `scoped()` / `get_scoped()` / `get_scoped_device()`; writes must use `write_scope` (400 "Select a client first" when none). Another client's object is **404, never 403**. Every new query on a client-owned table must be scoped, and every new client-owned table needs `client_id` in `models.py`, `ensure_default_client` (`CLIENT_OWNED`) and the tests in `tests/test_clients.py`.
  The hub scopes admin sockets and device pushes by client (`notify_admins(event, client_id, ...)`); `notify_admins` takes `event` positionally, so payload keys must not be called `event`. Zone/group names are unique per client (composite index; `sync_schema` drops the legacy global UNIQUE on SQLite by table rebuild and on Postgres by `DROP CONSTRAINT`).
  Suspended clients get 403 for users and devices (`security.py`). Enrollment keys tie discovery entries to a client. Deleting a client is refused while it owns live data and then also deletes its broadcasts/alerts/tamper events (Postgres enforces the FKs; SQLite does not, so run the suite on Postgres with `TEST_DATABASE_URL` after touching this).
- **Fleet inventory** (`services/fleet.py`, `services/versions.py`, `api/fleet.py`): agents report OS/arch/runtime/version/capabilities in register and heartbeat (`schemas.Inventory`, all optional so old agents work). Policy (`fleet_policy:<client>` setting) gives ok/outdated/unsupported/unknown; informational only.
- **Tamper-proofing** (`services/deviceauth.py`, `manifest.py`, `tamper.py`, `fleet.check_code`; agent `device/identity.py`): Ed25519 identity key bound at first registration (signed registration proves possession); afterwards `security.current_device` (async, reads the body) verifies `X-Signature/X-Timestamp/X-Nonce`
  over `METHOD\nPATH?QUERY\nTS\nNONCE\nsha256(body)`, ±120 s, nonce cache (in memory, single process). Bound devices can never fall back to unsigned; `DEVICE_AUTH_MODE=required` also refuses unbound ones. The agent signs the path *without* the `/api` prefix (that is what the backend sees) and uses the server's `Date` header as its clock.
  `/device/{id}/content` returns a `signed` envelope (server Ed25519 key, `SERVER_SIGNING_KEY` or stored in settings); the agent pins the key on first registration, plays only the signed payload, verifies each media sha256 and re-verifies the cache every 60 s. `services/tamper.record` writes a hash-chained `TamperEvent` (hash text uses UTC-naive times so SQLite and Postgres agree), flags the device (critical only), raises a `tamper` alert; response is **flag only**, admins clear it (`/devices/{id}/tamper/clear`).
  A changed key on a bound device is refused as `clone_attempt`; "Revoke and re-issue token" clears key, fingerprint and baseline. Agent state (`state.json`) is HMAC'd; the local display server binds 127.0.0.1, checks `Host`, and `/api/control` needs `DEMO_CONTROLS=1` plus a per-run token (`CONTROL_TOKEN` env when a launcher such as `run_all.py` must call it).
  Docker sim keeps keys in the `sim-cache` volume: losing the cache makes the server (correctly) refuse the "new machine" until the token is rotated. Bump `device/VERSION` (the single source of the agent version) when releasing; `make-device-kit.sh` prints the code hash for **Fleet > Trusted agent builds**.
- **Routes** (`models.Route`, `services/routes.py`, `services/geo.route_position`, `api/routes.py`): displays follow one route (`Device.route_id`); every `update_location` calls `routes.track`, which stores leg/progress/offset on the device, logs leg changes and raises/resolves an `off_route` alert (the health-alert code only touches `offline`/`health_low`, keep it that way). Assignments may carry `route_id` and `route_leg` (NULL = whole route); the resolver skips them for other routes/legs and counts the route as a specificity dimension. Legs are straight lines in a flat local frame (fine to a few km).
  Any PUT that rebuilds an `AssignmentIn` (e.g. the Schedules pause toggle) must send `route_id`/`route_leg` too, or it silently drops the route target.
- **Location history** (`services/tracking.py`, `models.LocationPoint`): a point only when the display first reports, enters another zone, or moved 50 m and 5 s; pruned hourly by the monitor loop (`TRACK_RETENTION_DAYS`). `entered` marks zone entries, which is what `/monitoring/zone-visits` counts.
- **Screenshots** (`api/device_api.py` `/device/screenshot`, `api/tracking.py`, `device/display/index.html` `capture()`): the display page redraws its content to a canvas and posts a JPEG to the local agent (`X-Page-Token`); the agent uploads it signed. The server keeps one per device in storage (`shot-<device pk>.jpg`), max `SCREENSHOT_MAX_KB`, 3 s rate limit. Capture now pushes `{"type":"sync","reason":"screenshot"}` over the device websocket. Needs the kiosk page open; it is a redraw, not real screen pixels.
- **Phone/tablet web display** (`frontend/public/display/`: `app.js`, `index.html`, `sw.js`, `manifest.webmanifest`): a static app (no build step, copied by Vite from `public/`) that re-implements the agent protocol in the browser: WebCrypto Ed25519 key (non-extractable, in IndexedDB), signed requests (same canonical string, path without `/api`, server clock from the `Date` header), signed-playlist verification with the pinned server key, sha256-checked media in IndexedDB, Geolocation or a fixed city, WebSocket sync, screenshots via canvas, and a service worker (`ignoreVary` is required or Vite/nginx `Vary` headers cause offline cache misses).
  It must stay in step with the Python agent: `VERSION` in `app.js` = `device/VERSION`; any change to signing, the playlist envelope or tamper kinds must be made in both. nginx serves it via `location /display/`; the Vite dev server needs the small `phoneDisplay` middleware in `vite.config.ts`, otherwise `/display/` returns the dashboard. Needs HTTPS or localhost. The browser profile is the "machine": clearing site data means a new key and a `clone_attempt` until the token is rotated. `window.__display` exposes read-only hooks for browser tests.
...
Tested: 94 backend tests (incl. `test_clients.py`, `test_tamper.py`, `test_tracking.py`, also green on PostgreSQL via `TEST_DATABASE_URL`); live tamper scenarios (modified cache, cloned identity, stolen token, edited state/code); clients/fleet/security/routes pages in headless Chromium; a live route drive with off-route alert and screenshots; the phone web display with an emulated Android phone (signed registration, GPS-driven content, offline reload, tamper); the previous version's DB upgraded on SQLite and Postgres; live agents (geofence switching, offline/reconnect, emergency applied on reconnect, impressions, WebM playback from MinIO); full Docker stack with the sim profile;
Pinggy tunnel through `tunnel.sh`; display-kit unzip + `start.sh` first run; wrong-token warning; browser regression. **Not tested:** the nginx `/display/` location (only the Vite dev route was run), the phone web display on a physical phone, iOS Safari or with the screen locked, tamper detections on Windows/Linux hardware, TPM/attestation (not built), the systemd/kiosk hardening checklist, the Docker sim with keys in a volume after a real restart, `DEVICE_AUTH_MODE=required` against live agents (unit-tested only), Firebase against a *real* project (only the Auth emulator + self-signed tokens: no real Google certs/consent/email), discovery across networks, health weights on real fleets, two physical laptops, Windows `start.bat`, Cloudflare connecting, ngrok success,
