...
export function DeviceDetailModal({ id, groups, onClose, onChanged }: { id: string; groups: Group[]; onClose: () => void; onChanged: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setCfg((c) => c ?? dev.data.config); setName((n) => n || dev.data.name); setGroup((g) => (g === '' && dev.data.group_id ? String(dev.data.group_id) : g))
  }, [id])
  useEffect(() => { load() }, [load])
  ...
  const save = async () => {
    try {
      await api.put(`/devices/${id}`, { name, config: cfg, group_id: group ? +group : null, clear_group: !group })
