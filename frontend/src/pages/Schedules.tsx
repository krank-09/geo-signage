...
export default function Schedules() {
  ...
  const toggle = (a: Assignment) => api.put(`/assignments/${a.id}`, {
    content_id: a.content_id, zone_id: a.zone_id, group_id: a.group_id, route_id: a.route_id, route_leg: a.route_leg, start_time: a.start_time, end_time: a.end_time,
    priority: a.priority, is_emergency: a.is_emergency, active: !a.active,
  }).then(load)

  return (
    <div className="stagger space-y-6">
      <PageHeader title="Schedules & assignments" subtitle="Emergency beats priority; higher priority beats lower; a more specific target (zone, group, route, route leg) beats “everywhere”. Equal winners play as a playlist. Time windows use server time (IST) and may cross midnight." />
      {admin && <Card title="New assignment"><AssignmentForm onDone={load} /></Card>}
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase text-ink-400"><tr><th className="py-2">Content</th><th>Zone</th><th>Route</th><th>Group</th><th>Window</th><th>Priority</th><th>State</th><th /></tr></thead>
            <tbody className="divide-y divide-ink-100/70">
              {items.map((a) => (
                <tr key={a.id} className={a.active ? '' : 'text-ink-400'}>
                  ...
                  <td>{a.zone_name || (a.route_name ? '—' : 'Everywhere')}</td><td>{a.route_name ? `${a.route_name}${a.route_leg != null ? ` · leg ${a.route_leg + 1}` : ''}` : '—'}</td><td>{a.group_name || 'All'}</td>
