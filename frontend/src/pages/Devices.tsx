      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase text-ink-400"><tr><th className="py-2">Device</th><th>Status</th><th>Group</th><th>Location</th><th>Zone</th><th>Now playing</th><th>Last seen</th></tr></thead>
            <tbody className="divide-y divide-ink-100/70">
              {devices.map((d) => (
                <tr key={d.device_id} onClick={() => setOpen(d.device_id)} className="cursor-pointer hover:bg-white/60">
                  ...
                  <td><StatusBadge status={d.status} /></td><td>{d.group || '—'}</td>
